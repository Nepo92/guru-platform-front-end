...
class AnalyticTableUtils {
  ...
  getCurrentRows(props: iCurrentProps) {
    ...
    if (current) {
      const currentVisible = current.filter(
        (el) => el.visible && el.visible !== undefined
      );
      ...
      const search = searchRow.value
        ? current.filter((el) =>
            el.name
              .toLowerCase()
              .trim()
              .includes(searchRow.value.toLowerCase().trim())
          )
        : false;
  ...
  #getMonthsNames(props: iGetPeriodItemsProps) {
    ...
    if (periodLength) {
      ...
      const monthsNames = months.map((item, index) => {
  ...
  #getDaysNames(props: iGetPeriodItemsProps) {
    ...
    if (periodLength) {
      for (let index = 1; index < periodLength; index++) {
        names.push(
          dateUtils.formatDDMMYYYY(start + index * 24 * 60 * 60 * 1000)
        );
      }
