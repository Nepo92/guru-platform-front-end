...
export default defineComponent({
  emits: ["create-funnel-color-settings"],
  setup(props, { emit }) {
    let form = ref({} as HTMLFormElement);
    const { filter } = store;

    const [ratingColors, kpdColors, proceedsColors, mailingColors] =
      filter.lineColors;

    onMounted(() => {
      emit("create-funnel-color-settings", form.value);
    });
