...
export default {
  ...
  created() {
    ...
    const isManager = this.role === 'ROLE_MANAGER' || this.role === 'ROLE_HEAD_MANAGER' ? this.itemsManager : false;
    ...
    this.menuItems = isAdmin || isManager || isExaminer || isAdvertiser || isHeadManager || isCurator;
  },
  methods: {
    openMenu() {
    ...
    openSubMenu(index) {
      ...
      const checkIndex = currentIndex === index || currentIndex === null;

      const openWithAnimation = checkIndex && isOpen && !prevIsOpen ? 'open' : false;
      const openWithoutAnimation = checkIndex && isOpen && prevIsOpen ? 'open_t0' : false;

      const open = openWithAnimation || openWithoutAnimation;

      const closeWithAnimation = checkIndex && !prevIsOpen && !isOpen && !mouseEnter ? 'close' : false;
      const closeSubMenu = checkIndex && mouseEnter ? 'close' : false;
...
<template>
  <nav
    ref="menu"
    class="menu"
    @mouseenter.stop="openMenu"
    @mouseleave="closeMenu"
  >
    <ul class="menu__wrapper">
      <li class="menu__company menu-company">
        <span class="menu-company__open">
          <div class="menu-open__row" />
        </span>
        <img
          class="menu-company__img"
          :src="company.logo"
        >
      ...
      <li
        class="menu__list menu-list custom-scroll"
        :class="checkOverFlowMenuItems"
      >
        <ul>
          <li
          ...
          >
            <a
              ...
              :href="item.path"
              @mouseenter.stop="(e) => hoverMenuItem(e, index)"
            ...
            <ul
              ...
              ref="submenu"
              class="sub-menu"
              :class="openSubMenu(index)"
              @mouseenter="(e) => mouseOnSubMenu(e)"
            >
              <li
                v-for="(elem, count) of item.submenu"
                :key="count"
      ...
      <li
        class="menu__logout"
        @mouseover.self="openMenu"
      >
        <a
          class="menu-logout__link logout"
          href="/logout"
        >Выйти</a>
      </li>
    </ul>
