...
function initVideoItem() {
  ...
  $.each($(".content-video__preview"), (index, item) => {
    $(item).on("load", function () {
      const playBtn = $(this)
        .parent()
        .children(".content-video__play-btn")
        .css({ display: "block" });
    });
  });
}
...
function setEstimationToLesson() {
  if ($(".estimation__text--client").text() < 4) {
    $(".estimation__icon--client").css({
      maskImage: "url(/img/frown.svg)",
      backgroundColor: "#F5244A",
    });
  } else if (
    $(".estimation__text--client").text() > 3 &&
    $(".estimation__text--client").text() < 7
  ) {
    $(".estimation__icon--client").css({
      maskImage: "url(/img/meh.svg)",
      backgroundColor: "#FFA515",
    });
  } else if ($(".estimation__text--client").text() >= 7) {
    $(".estimation__icon--client").css({
      maskImage: "url(/img/smile.svg)",
