...
$(document).on('submit', '[js-menu-search-form]', function (event) {
    ...
    if ($.trim(searchValue) !== '') {
        ...
        if ($('[js-menu-search-input]').val() === '+7' || $('[js-menu-search-input]').val() === '8') {
...
function openBills(formData) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json",
        url: "/api/bills/getHeadManagerBills",
        data: JSON.stringify(formData),
        dataType: 'json',
        cache: false,
        success: function (data) {
...
$(document).on('submit', '[js-deal-form]', function (event) {
    ...
    if (validateForm(this)) {
        ...
        dealData.idFunnel = +$('[deal-type] option:selected').attr('value');
