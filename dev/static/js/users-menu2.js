...
$(document).ready(function () {
    ...
    $menuCreateManagerCloseBtn.on('click', function () {
        const wrapper = $menuCreateManager.get(0).querySelector('.platform-modal__wrapper');
    ...
    $menuCreateAdvertiserCloseBtn.on('click', function () {
        const wrapper = $menuCreateAdvertiser.get(0).querySelector('.platform-modal__wrapper');
    ...
    $managersListCloseBtn.on('click', function () {
        const wrapper = $managersListMenu.get(0).querySelector('.platform-modal__wrapper');
        closeModalAnimation($managersListMenu.get(0), wrapper, false, false, false);
...
function createAdvertiser(userData) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const menu = $('[js-menu-create-advertiser]').get(0)
...
function createManager(userData) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const menu = $('[js-menu-create-manager]').get(0)
...
function addManagerToTable(manager) {
    ...
    const managerRow = templateRowManager(manager, seloption);

    table.get(0).children[0].insertAdjacentHTML('afterend', managerRow);

    initDateUserPicker();
}

function templateRowManager(manager, seloption) {
    return '<tr data-manager="' + manager.id + '" department="manager" class="custom-table__body-row staff-table__body-row">' +
    '                        <input type="hidden" u-id value="' + manager.id + '" name="id" required>' +
    '                        <input type="hidden" u-id-manager value="' + manager.idManager + '" required>' +
    '                        <td class="custom-table__body-col staff-table__id" >' + manager.idManager + '</td>' +
    '                        <td class="custom-table__body-col staff-table__avatar">' +
    ...
    '                            </div>' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_xlarge staff-table__name" >' +
    ...
    '                                   placeholder="—">' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_xlarge staff-table__login">' +
    ...
    '                                   required>' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_medium staff-table__password">' +
    ...
    '                                   required>' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_medium staff-table__tel">' +
    ...
    '                                   placeholder="—">' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_medium staff-table__vk">' +
    ...
    '                                   placeholder="—">' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_medium staff-table__card">' +
    ...
    '                                   placeholder="—">' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_small staff-table__birthday">' +
    '                            <input u-birthday' +
    ...
    '                                   placeholder="—">' +
    '                        </td>' +
    '                        <td class="custom-table__body-col staff-table__key">' +
    ...
    '                            </div>' +
    '                        </td>' +
    '                        <td class="custom-table__body-col staff-table__monitor">' +
    '                           <div class="input-element__checkbox input-element__checkbox_mid">' +
    '                               <input u-isneedshow' +
    '                                      name="isNeedShow"' +
    '                                      type="checkbox"' +
    ...
    '                                      disabled' +
    '                                      id="showM' + manager.id + '' +
    '                                      checked="' + manager.isNeedShow + '">' +
    '                               <label class="checkbox-label checkbox-label_gray" th:for="showM' + manager.id  + '"></label>' +
    '                           </div>' +
    '                       </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_medium staff-table__position">' +
    ...
    '                            </div>' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_medium staff-table__status">' +
    ...
    '                            </div>' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_small staff-table__projects">' +
    '                            <div js-set-projects="" class="setting-wrapper">' +
    '                                   <div class="theme-setting theme-setting_big theme-setting_m"></div>' +
    '                           </div>' +
    '                        </td>' +
    '                        <td class="custom-table__body-col custom-table__body-col_last staff-table__settings">' +
...
function addAdvertiserToTable(advertiser) {
    ...
    const advTemplate = advertiserTemplate(advertiser, seloption);

    table.get(0).children[0].insertAdjacentHTML('afterend', advTemplate);

    initDateUserPicker();
}

function advertiserTemplate(advertiser, seloption) {
    return '<tr data-advertiser="' + advertiser.id + '" department="advertiser" class="custom-table__body-row staff-table__body-row" >' +
    '                        <input type="hidden" u-id value="' + advertiser.id + '" name="id" required>' +
    '                        <td class="custom-table__body-col staff-table__id" >' + advertiser.idManager + '</td>' +
    '                        <td class="custom-table__body-col staff-table__avatar">' +
    '                            <div class="avatar-upload">' +
    '                                <form u-upload-avatar-form action="#" enctype="multipart/form-data">' +
    ...
    // '                            </div>' +
    // '                        </td>' +
    '                           <td class="custom-table__body-col custom-table__body-col_small staff-table__access">' +
    '                            <div class="input-element__checkbox input-element__checkbox_mid">' +
    '                                <input u-access' +
    '                                       name="access"' +
    '                                       type="checkbox"' +
