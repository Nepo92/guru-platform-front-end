...
class Utils {
  ...
  isReminderBtn(t) {
    ...
    return (
      (reminderWrapper && reminderWrapper.hasAttribute("data-deal-reminder")) ||
      t.hasAttribute("data-deal-reminder")
    );
  }
  ...
  getPage(param) {
    ...
    return !param
      ? this.getPageName(page, param)
      : page[page.length - (param + 1)];
  }
  ...
  getPageName(page) {
    ...
    const exception =
      (isNaN(last) && banned.includes(last)) ||
      last.split("?").length > 1 ||
      last.split("#").length > 1;
  ...
  accountNumberRequire(props, e) {
    ...
    menu = menu.hasAttribute("[add-bill-menu]")
      ? menu
      : document.querySelector("[add-bill-menu]");
  ...
  toggleCreateMenu(menu, t, items, props) {
    ...
    const name = this.getParent(t, "bills-form__item").querySelector(
      ".bills-item__name"
    );
  ...
  getDateFormatDDMMYYYY(date, spliter = ".") {
    ...
    const badDayFormat =
      dateArr[0] < 10 && dateArr[0].length < 2 ? "bad-day" : "";
    const isNotDay = dateArr[0].length > 2 ? "bad-day" : "";

    const badDay = isNotDay || badDayFormat;

    const badMonth =
      dateArr[1] < 10 && dateArr[1].length === 1 ? "bad-month" : "";

    const badYear =
      dateArr && dateArr[2].length !== 4 && dateArr[0].length === 2
        ? "bad-year"
        : "";
    ...
    switch (formattedDate) {
      ...
      case "bad-day": {
        ...
        if (dateArr[0].length !== 4) {
          let day =
            dateArr[0] < 10 && dateArr[0].length < 2
              ? `0${dateArr[0]}`
              : dateArr[0];
          ...
          let day =
            dateArr[2] < 10 && dateArr[2].length < 2
              ? `0${dateArr[2]}`
              : dateArr[2];
  ...
  getSymboRoubles(key, value, withoutNull = false) {
    const roubles = [
      ...
      "advExpenses",
      "average",
      "averageTrafficMailing",
      "averageAdditional",
      "profit",
      "revenue",
      "averageAdditionalPage",
      "revenueTraffic",
      "advExpensesTrafficPage",
      ,
      "averageCheck",
      "clickPrice",
      "applicationPrice",
    ...
    const percent = [
      "shareAdvExpensesTrafficPage",
      "CV5Row",
      "clickToApplicationDo",
      "applicationToClient",
      "invoiceToClient",
      "applicationToInvoice",
      "clickToApplicatio",
      "showToClick",
      "prescribedToSale",
      "clickToApplication",
    ...
    return !withoutNull
      ? roubles.includes(key)
        ? value === null
          ? `0 ₽`
          : `${numbers.setWhiteSpace(value)} ₽`
  ...
  sortingArrayTasks(array) {
    ...
    switch (notEmptyToday) {
      case true: {
        ...
        const notTodayExpiration = notTodayNotDone.filter(
          (el) => el.reminderExpiration
        );
        ...
        const notTodayNotExpiration = notTodayNotDone.filter(
          (el) => !el.reminderExpiration
        );
  ...
  checkClientCardOpen() {
    return document
      .querySelector("[js-menu-client-card]")
  ...
  compareDates(one, two) {
    ...
    const oneArr =
      oneFormatted.split(".").length > 1
        ? oneFormatted.split(".")
        : oneFormatted.split("-");
    const twoArr =
      twoFormatted.split(".").length > 1
        ? twoFormatted.split(".")
        : twoFormatted.split("-");
  ...
  sortBills(one, two) {
    const first = new Date(
      "20" + one.plannedPayDate?.split(".")[2],
      one.plannedPayDate?.split(".")[1],
      one.plannedPayDate?.split(".")[0]
    );
    const second = new Date(
      "20" + two.plannedPayDate?.split(".")[2],
      two.plannedPayDate?.split(".")[1],
      two.plannedPayDate?.split(".")[0]
    );
  ...
  sumBills(bill) {
    ...
    const bills = Array.from(billsItems).filter(
      (el) =>
        !el.classList.contains("bill__create") &&
        el.getAttribute("data-bill") > 0 &&
        (bill ? +el.getAttribute("data-bill") !== bill.id : true)
    );
