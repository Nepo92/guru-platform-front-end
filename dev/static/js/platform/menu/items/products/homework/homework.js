...
class Homework {
	init() {
		...
		if (items) {
			...
			const saveHomework = document.querySelector('.homework-comments__send--btn');

			if (saveHomework) {
				const uploadBtns = document.querySelectorAll('.homework-comment__input--file');
			...
			function afterSaveHomework(data, form) {
				...
				const commentsWrapper = document.querySelector('.homework-comments__wrapper');
				...
				notifyText.innerText = 'Спасибо! Ваше домашнее задание принято на проверку, ответ получите в скором времени!';
		...
		function renderComments(comments, rate) {
			if (comments.comments.length) {
				...
				const commentItems = comments.comments.sort((a, b) => b.id - a.id).map((item, count) => {
					...
					return `
						<li class="homework-comments__item ${item.type === 'admin' ? 'admin' : ''}">
							<div class="comments-item__wrapper">
								<p>${item.text}</p>
									${files.length ?
							`<ul class="homework-comments__gallery custom-scroll ${files.length ? 'full' : ''}">
											${files.map((file, index) => setCommentFiles(file, index, false)).join('')}
										</ul>` : ''
						}

									${count == comments.comments.length - 1 && rate ? `<div class="homework-rating">Оценка: ${rateInComment(rate)}</div>` : ''}
							</div>
						</li>
					`
				}).join('');
		...
		function rateInComment(rate) {
			...
			return rates.map((item, index) => {
				...
				return isFull ? `<span class="homework-rate__score ${index === 0 ? 'ml_5' : '' }"></span>` : '<span class="homework-rate__score-empty"></span>';
			}).join('');
		}
		...
		function homework(items, table, menu, preloads) {
			if (items && table && menu) {
				...
				function templateHomeworkrow(item) {
					return `
            <td class="platform-table__column homework-table__column">${item.homework.createdDate ? item.homework.createdDate : ''}</td>
            <td js-client-id class="platform-table__column homework-table__column">
                <a href="" class="homework-table__link" js-client-card title="${item.client.name}">${item.client.name ? item.client.name : ''}</a>
            </td>
            <td class="platform-table__column homework-table__column">${item.manager.name ? item.manager.name : ''}</td>
            <td class="platform-table__column homework-table__column">
                <a href="" title="${item.course.name}" js-course-btn class="homework-table__link">${item.course.name ? item.course.name : ''}</a>
                <p class="homework-table__course-type">${item.course.courseType ? item.course.courseType : ''}</p>
            </td>
            <td class="platform-table__column homework-table__column">
                <a href="" js-module class="homework-table__link" title="${item.module.index + ' | ' + item.module.title}">${item.module.index + ' | ' + item.module.title}</a>
            </td>
            <td class="platform-table__column homework-table__column">${item.lessonRate ? item.lessonRate : 'Нет'}</td>
            <td class="platform-table__column homework-table__column">${item.stream.id ? item.stream.id : ''}</td>
            <td class="platform-table__column homework-table__column">${item.stream.startDate ? item.stream.startDate : ''}</td>
            <td class="platform-table__column homework-table__column homework__rate" estimation>
                ...
                <span class="homework__rate--value">${item.homework.rate ? item.homework.rate : 'Нет'}</span>
            </td>
            <td rate class="platform-table__column homework-table__column">
                <div rate-homework class="homework-rate 
                    ${(item.homework.rate == 0 && item.homework.checker == null) ? 'homework-rate__check' :
							(item.homework.rate !== 0) ? 'homework-rate__view' :
								(item.homework.rate == 0 && item.homework.checker != null) ? 'homework-rate__check' : ''
						}">
                    ${(item.homework.rate == 0 && item.homework.checker == null) ? 'Проверить' :
							(item.homework.rate !== 0) ? 'Проверено' :
								(item.homework.rate == 0 && item.homework.checker != null) ? 'Пересдача' : ''}
                </div>
            </td>
            <td checker class="platform-table__column homework-table__column">${item.homework.checker ? item.homework.checker : ''}</td>
        `
				}
				...
				renderTable(items, table).then(() => {
					...
					const uploadBtns = menu.querySelectorAll('.homework-comment__input--file');

					uploadBtns.forEach((item) => {
						if (item.getAttribute('id') === 'imageFile') {
							const uploadClone = getCloneBefore(item);
							const uploadFilesForResponse = uploadFiles.bind(this, preloads, menu);
		...
		function getHomework(menu, json, isNeedFormBlocks) {
			...
			getHomeworkData(json).then((data) => openHomework(menu, data, isNeedFormBlocks).then(() => {
				...
				const responseHomework = document.querySelector('.homework-menu__response');
		...
		function clearHomeworkForm() {
			const rates = document.querySelectorAll('.homework__estimation--checkbox');
			...
			const inputs = document.querySelectorAll('.homework-comment__input--file');
		...
		function setHomeworkComments(item) {
			...
			return `
        <div class="comments-item__wrapper ${item.type === 'admin' ? 'admin' : ''}">
            ${item.text}
            <ul class="homework-comments__gallery custom-scroll ${item.homeworkFiles.length ? 'full' : 'empty'}">
                ${fileItems.join('')}
            </ul>
        </div>
    `
		...
		function setCommentFiles(file, index, isNew) {
			...
			file.name.split('.').splice(file.name.split('.').length - 1, 1).join('');
			...
			return `
            <li ${isNew ? `data-index="${index}"` : ''} class="comments-gallery__file">
                ${isImage ? `
                    <a data-fancybox href="${isNew ? `${path}` : `/${file.path}`}" class="comments-gallery__link">
                        <img class="comments-gallery__img" src="${isNew ? `${path}` : `/${file.path}`}">
                        ${isNew ? `<span class="comments-img__delete"></span>` : ''}
                    </a>`
					:
					`<a class="comments-gallery__link ${isNew ? `${type}` : `${file.type}`}" downloads href="${isNew ? `${path}` : `/${file.path}`}">
                        <span class="comments-gallery__type">${isNew ? `${type}` : `${file.type}`}</span>
		...
		function setRateStarInComment(menu, isEmpty) {
			...
			const span = document.createElement('span');
			span.classList.add(isEmpty ? 'homework-rate__score-empty' : 'homework-rate__score');
			span.classList.add(isEmpty ? 'homework-menu__score-impty-icon' : 'homework-menu__score-icon');
			...
			$.each(homeworkRow, (index, item) => {
				if (+$(item).attr('data-homework') === data.id) {
					...
					if (data.rate !== 0) {
						...
						ratingCol.append($('<span/>').attr('class', 'homework-rate__text').html(data.rate));
					} else {
						ratingCol.text('Нет');
					}

					$(item).find('.btn_title-text_small').html(data.rate == 0 ? (data.checker ? 'Пересдача' : 'Проверить') : 'Проверено');
					...
					if (data.accepted) {
						...
						const btnClasses = $(item).find('[rate-homework]').css({ borderRadius: '5px' });
						$(btnClasses).removeClass('btn__content');
		...
		function uploadFiles(preloads, menu, e) {
			...
			if (t.value) {
				...
				if (preloads.length > 8) {
					...
					const inputs = menu.querySelectorAll('.homework-comment__input--file');
					inputs.forEach((item) => {
						item.value = '';
					})
				}
		...
		function getGallery() {
			...
			const commentsGallery = document.querySelectorAll('.homework-comments__gallery');

			if (commentsGallery.length) {
				const galleryItems = document.querySelectorAll('.homework-comments__gallery');
				gallery = galleryItems[galleryItems.length - 1];
			}

			const adminGallery = document.querySelector('.homework-comment__gallery--admin');

			if (adminGallery) {
				gallery = document.querySelector('.homework-comment__gallery--admin');
			}

			const clientGallery = document.querySelector('.homework-comment__gallery--client');
		...
		function setGalleryFiles(preloads, wrapper) {
			...
			const setPreloads = async function setPreloadsFiles(preloads, wrapper) {
				for (let i = 0; i < preloads.length; i++) {
					...
					if (!file.path) {
						path = URL.createObjectURL(file);
						type = file.type.split('/')[0] === 'image' ? file.type.split('/')[0] : file.name.split('.')[file.name.split('.').length - 1];
					} else {
						path = file.path;
						type = file.type;
					}
					...
					if (type === 'image' || type === 'jpg' || type === 'jpeg' || type === 'png') {
						...
						if (wrapper) {
							const galleryComment = wrapper.querySelector('.homework-comments__gallery')
		...
		$(document).on('click', '.homework-estimation__label--accept', function (e) {
			...
			if (!t.classList.contains('accept')) {
				...
				const decline = document.querySelector('.homework-estimation__label--decline');
		...
		$(document).on('click', '.homework-estimation__label--decline', function (e) {
			const block = document.querySelector('.homework-estimation__block');
			const input = document.querySelector('[accepted]');
			const t = e.target;
			...
			if (!t.classList.contains('decline')) {
				...
				input.value = Boolean('');
				const accept = document.querySelector('.homework-estimation__label--accept');
		...
		function setRate(accepted) {
			const rated = document.querySelectorAll('.homework__estimation--checkbox');
		...
		function setFilesToDataSend(data, preloads) {
			...
			const inputName = document.querySelector('.homework-comment__input--file').getAttribute('data-name');
		...
		function updateAfterSend(data, t, settings = 'admin') {
			if (settings === 'admin') {
				const deal = document.querySelector(`.platform-table__row[data-homework="${data.id}"]`);

				const rate = Array.from(deal.children).filter((el) => el.hasAttribute('rate'))[0];
				const rateBtn = Array.from(rate.children).filter((el) => el.hasAttribute('rate-homework'))[0];
				...
				const checker = Array.from(deal.children).filter((el) => el.hasAttribute('checker'))[0];
				checker.innerText = data.checker;

				const estimation = Array.from(deal.children).filter((el) => el.hasAttribute('estimation'))[0];
		...
		function clearHomeworkMenu(menu, preloads) {
			...
			const homeworkCheckedBtn = menu.querySelector('.homework-estimation__label--accept');
			homeworkCheckedBtn.classList.remove('accept');

			const homeworkUncheckedBtn = menu.querySelector('.homework-estimation__label--decline');
			...
			const textAreaGallery = menu.querySelector('.homework-comment__gallery--admin');
			...
			const inputFiles = menu.querySelectorAll('.homework-comment__input--file');
