...
class Marketing {
  ...
  marketingSettings() {
    ...
    $(document).on('click', '[update-table]', (e) => {
      e.preventDefault();
      e.stopPropagation();

      showLoader()
      const closeButton = this.#closeButton.bind(this);
      const removePopup = this.#removePopup.bind(this);
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          const popupProps = {
            ...
            settings: 'copy',
            title: 'Логи',
            contentCreator: closeButton,
          }

          const setPopup = popup.init(popupProps)
    ...
    const checkBtn = document.querySelector('.adv-nav__right--check-auth');
  ...
  #closeButton() {
    return '<div class="dialog__close"></div>'
  }

  #removePopup() {
    document.querySelector('[dialog-window]')?.remove();
  }
}
