...
class DealTemplates {
  dealRow(renderData) {
    ...
    return `
      ...
      <td course-row class="platform-table__column course">
        ...
        <span class="course__start course-crm mt_5">
          <span class="course-crm--icon">
            <div class="tooltip tooltip-course__start">Дата старта</div>
          </span>
          ${stream}
        </span>
      </td>
      <td class="platform-table__column price ${(deal.price === 0 && deal.paid === 0) ? 'center-content' : ''}">
        ${(deal.price === 0 && deal.paid === 0)
          ? `<div class="price-info">
              <div class="column_price"><b>0 ₽</b></div>
            </div>` : `<div class="price-info ${deal.price <= deal.paid ? 'green' : ''}">
