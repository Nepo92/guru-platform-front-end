  ...
  #dispatchSelectItem(selectProps) {
    ...
    switch (type) {
      ...
      case 'select-deal-type': {
        ...
        if (deal) {
          const isAdditional = deal.type === 'Допродажа' || deal.type === 'База';

          const value = isAdditional ? 'additional' : 'traffic';
          selectItemProps.defaultValue = value;
          selectItemProps.defaultPlaceholder = deal.type === 'Допродажа' || deal.type === 'additional' ? 'База' : 'Трафик';
        }
        break;
      }
      case 'select-funnel': {
