...
class Actions {
  init() {
    ...
    if (utils.getPage() === 'action') {
      console.log(utils.getPage() === 'action');
      ...
      const removeAction = function (id) {
        const remove = async () => {
          return await $.ajax({
            type: 'GET',
        ...
        remove().then(() => {
          const row = $(`[value="${id}"]`).closest('.custom-table__body-row');
          row.remove();
        }).catch(() => {
          const row = $(`[value="${id}"]`).closest('.custom-table__body-row');
          row.remove();
      ...
      $(document).on('click', '.btn_delete', function (e) {
        ...
        const id = +$(t).closest('.body__cell').find('[type="hidden"]').val();
        const remove = removeAction.bind(removeAction, id);
