...
class Control {
  init() {
    ...
    $(document).on('click', '[open-rate]', function (event) {
      const idTemplate = $(this).data('rate');
      openRateMenu(idTemplate);
    });
    ...
    function openAddRateMenu(idDeal) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $(menu).find('[js-save-rate]').removeClass('hide');
          $(menu).find('.element-delim').removeClass('hide');
          $(menu).find('.score').removeClass('hide');
    ...
    function setTemplateItems(data) {
      ...
      $.each(data, function (index, item) {
        row += '<tr class="platform-table__row">' +
          ...
          '               <input type="radio" id="good' + index + '" class="platform__checkbox radio radio_icon radio_like rate-value" name="items[' + index + '].score" value="' + item.goodScore + '" required>                   ' +
          '               <label for="good' + index + '" class="rate-block__label rate-block__label--good label"> ' +
          '                   <span class="rate-block__label--text-good">Отлично</span> ' +
          '               </label>               ' +
          '           </div>               ' +
          '           <div class="rate-block__btn rate-block__btn_small">                   ' +
          '               <input type="radio" id="bad' + index + '" class="platform__checkbox radio radio_icon radio_dislike rate-value" name="items[' + index + '].score" value="0" required>                   ' +
          '               <label for="bad' + index + '" class="rate-block__label rate-block__label--bad label"> ' +
          '                   <span class="rate-block__label--text-bad">Плохо</span> ' +
          '               </label>               ' +
          '           </div>           ' +
          '       </div>       ' +
          '   </div>' +
    ...
    $(document).on('click', '[view-comment]', function (event) {
      if ($(this).attr('data-rate')) {
        openCommentMenu($(this).attr('data-rate'));
      } else {
    ...
    $(document).on('click', '[js-menu-open-comment-close-btn]', function (event) {
      const menu = document.querySelector('[js-menu-open-comment]');
      const wrapper = menu.querySelector('.platform-modal__wrapper');
      closeModalAnimation(menu, wrapper, false, false, false, false);
    ...
    function openRateMenu(idRate) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          const wrapper = Array.from(document.querySelector('[rate-wrapper-items]').children);
          ...
          $(menu).find('[js-save-rate]').addClass('hide');
          $(menu).find('.element-delim').addClass('hide');
          $(menu).find('.score').addClass('hide');

          setRateInfo(menu, data);
          checkBodyHidden();
    ...
    function setRateInfo(menu, data) {
      const wrapper = menu.querySelector('[rate-wrapper-items]');

      data.items.forEach((item, index) => {
        const tr = document.createElement('tr');
        tr.classList.add('platform-table__row');
        tr.innerHTML = getRateTemplate(item, index);
        wrapper.appendChild(tr);
      })
    }

    function getRateTemplate(item, index) {
      return `
            <td class="platform-table__column rate-point-item">${item.itemName}</td>
            <td class="platform-table__column rate-values">${item.score}</td>
            <td class="platform-table__column rate-btn">
                ${item.score ?
          `<span view-comment data-rate="${item.id}" class="platform-form__access-btn mt_0">Посмотреть</span>`
          : ''
        }
            </td>
        `
    ...
    function renderRate(data) {
      ...
      if (score.length) {
        ...
        if (estimationWrapper.length) {
          const rateWrapper = estimationWrapper.filter((el) => el.classList.contains('estimation-column'))[0];
    ...
    $(document).on('mouseover', '.estimation-column', getRate);
    ...
    function getRate(e) {
      ...
      const rateInfo = setRateInfoInTooltip.bind(this, e);
    ...
    function setRateInfoInTooltip(e, data) {
    ...
    $(document).on('click', '.platform__close--btn', function (e) {
      ...
      const menu = getParent(t, 'platform-modal');
      const wrapper = menu.querySelector('.platform-modal__wrapper');
      closeModalAnimation(menu, wrapper, false, true);
    })
    ...
    function appendTooltipToCRM(data) {
      const tooltipTemplate = getTooltipTemplate(data);

      const row = document.querySelector(`.platform-table__row[data-deal="${data.idDeal}"]`);

      row.querySelector('.estimation-column .column-text').insertAdjacentHTML('afterbegin', tooltipTemplate);
    }
    ...
    function getTooltipTemplate(data) {
      const templates = data.items.map((item) => {
        return `
                <tr class="c-tooltip-table__body-col c-tooltip__text inherit">
                    <td class="c-tooltip-table__body-col c-tooltip__text inherit">${item.itemName}</td>
                    <td class="c-tooltip-table__body-col c-tooltip__text inherit">${item.score}</td>
                </tr>
            `;
      });

      return `
            <div class="tooltip tooltip-estimation">
                <table class="c-tooltip-table">
                    <thead class="c-tooltip-table__head">
                        <tr class="c-tooltip-table__head-row">
                            <td class="c-tooltip-table__head-col c-tooltip__text c-tooltip__text_title">Список</td>
