                            ...
                            .append($("<div/>").attr("class", "deal-card__inner")
                                .append($("<div/>").attr("class", "deal-card__header").attr('style', (!value.backgroundColor) ? `background-color: #ffffff` : `background-color: ${value.backgroundColor}`)
                                    ...
                                    .append($("<div/>").attr("class", "deal-card__task deal-task")
                                        ...
                                        (value.reminders[value.reminders.length - 1].done) ? "deal__reminder-title deal-reminder deal-reminder_green " : "deal__reminder-title deal-reminder deal-reminder_red" : '')
                                        .html((value.reminders.length !== 0) ? (value.reminders[value.reminders.length - 1].reminder) ? value.reminders[value.reminders.length - 1].reminderDate : "Нет задач" : 'Нет задач'))
...
function loadBlocks(idClient, idDeal) {
    $.ajax({
        type: "POST",
        contentType: "application/json",
        ...
        success: function (modules) {
            ...
            $.ajax({
                ...
                success: function (blocks) {
                    if (blocks.length !== 0) {
                        renderBlocksToCard(blocks, idDeal);
                        setModulesToBlocksCard(idDeal, blocks, modules);
                    } else {
                        setModulesToBlocksCard(idDeal, blocks, modules);
                    }
...
function setModulesToBlocksCard(idDeal, blocks = null, modules) {
    if (blocks.length !== 0) {
        $.each(blocks, (index, item) => {
            const modulesInBlock = modules.modules.filter((el) => item.courseBlock.moduleIds.includes(el.idModule));

            const modulesItems = $.map(modulesInBlock, (elem, count) => {
                return `
                            <div js-module="${elem.idModule}" js-module-deal="${idDeal}" js-module-client="${$CLIENT}" class="deal__lessons-item ${elem.enabled ? 'deal__lessons-item_active' : ''}">${count + 1}</div>
                        `
            });

            $.each($('.lesson-block'), (count, elem) => {
                if (item.courseBlock.id === $(elem).data('id') && idDeal === +$(elem).closest('.deal-card').attr('dealid')) {
                    $(elem).find('.lesson-block__modules').children().remove();
                    $(elem).find('.lesson-block__modules').append(modulesItems);
                }
            });
        });
    }
    ...
    if (blocks.length === 0) {
        const modulesItems = $.map(modules.modules, (elem, count) => {
            return `
                <div js-module="${elem.idModule}" js-module-deal="${idDeal}" js-module-client="${$CLIENT}" class="deal__lessons-item ${elem.enabled ? 'deal__lessons-item_active' : ''}">${count + 1}</div>
            `;
        });

        $.each($('.deal-card'), (index, item) => {
            if (+$(item).attr('dealid') === idDeal) {
                $(item).find('.deal__lessons').append(modulesItems);
            }
        })
    }
}

function renderBlocksToCard(blocks, idDeal) {
    const block = $.map(blocks, (item, index) => {
        return `
    ...
    $.each($('.deal-card'), (index, item) => {
        if (+$(item).attr('dealid') === idDeal) {
            $(item).find('.deal__lessons').append(block);
        }
    })
}
...
$(document).on('click', '.lesson-block__closed-icon', function (e) {
    ...
    $.ajax({
        ...
        data: JSON.stringify(clientCourseBlock),
        contentType: "application/json",
        dataType: 'json',
        cache: false,
        success: function () {
  ...
  $(document).on('click', '.deal__lessons-item', function (event) {
    const blocks = $(event.target).closest('.lesson-block').get(0);
  
    if (blocks) {
        if (!$(event.target).closest('.lesson-block').hasClass('active')) {
            return false;
        }
    }
