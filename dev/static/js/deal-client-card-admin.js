...
function setStatusDeals(data, statuses) {
    $('[check-target-status]').children().remove();

    const dealStatus = data.status;

    const status = $.map(statuses, (item, index) => {
        return `
      <option value="${item.id}" ${dealStatus === item.id ? 'selected' : ''}>${item.title}</option>
    `;
    });

    $('[check-target-status]').append(status);
}
...
$(document).on('click', '[js-client-card]', function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            clientInfo = data;
            dealAPI.getStatusDeals();
            setClientCardInfo(data);
            dealAPI.getFunnels(filter.idCompany);
            dealAPI.getProductMap();
            dealAPI.getCourseKindList();
            dealAPI.getCoursesList();
            dealAPI.getSocials();
...
function updateHideForm(idDeal, isHide) {
    ...
    if (isHide) {
        ...
        btn.removeClass('deal-card__delete').addClass('deal-card__recover');
        ...
        btn.addClass('deal-card__delete').removeClass('deal-card__recover');
...
function setDataStart(courseId, startDate) {

    const formData = new FormData();

    formData.set('idCourse', courseId);
    ...
    $.ajax({
        ...
        cache: false,
        success: function (data) {
            $('[js-update-form-deal-start-date]').children().remove();
            ...
            const start = startDate.split('.').splice(0, 2).join('.') + '.' + date.split('').splice(0, 2).join('') + startDate.split('.')[2];

            $.each($('[js-update-form-deal-start-date]').children(), (index, item) => {
                if ($(item).html() === start) {
    ...
    $.ajax({
        ...
        success: function (tasks) {
            const sortTasks = sortingArray(tasks);
...
function setDealInfo(data) {
    ...
    $.ajax({
        ...
        cache: false,
        success: function (data) {
            $('[js-course-btn]').prop('disabed', false);
            setUpdateDealInfo(data);
            setStatusDeals(data, $statusesList);
            setSocialToDeals(data, $socials);
            setSaleTypes($saleTypes);
            setCourses(data, $coursesList);
            $('[js-update-deal-btn]').prop("disabled", false);
            setDataStart(data.idProduct, data.startDate);
...
function setCourses(data, courses) {
    $('[js-update-form-deal-pr]').children().remove();

    const course = $.map(courses, (item, index) => {
        return `
      <option ${data.course === item.name ? 'selected' : ''} data-id="${item.id}" value="${item.name}" data-type="${item.type}" data-price="${item.price}">${item.name}</option>
    `;
    });
...
function setSocialToDeals(data, socials) {
    $('[check-social]').children().remove();

    const social = $.map(socials, (item, index) => {
        return `
      <option value="${item.id}" ${item.id === data.social ? 'selected' : ''}>${item.title}</option>
    `
    });

    $('[check-social]').append(social);
}
...
$(document).on('submit', '[js-update-form-deal]', function (event) {
    ...
    if (validateForm(this)) {
        ...
        formData.idProduct = $(this).closest('form').find('[js-update-form-deal-pr] option:selected').data('id');
...
$(document).on('click', '[js-add-bill-button]', function (e) {
    e.preventDefault();
    $('[js-menu-bills]').css({zIndex: 10000});
    $(this).closest('[js-add-bill-form]').trigger('submit');
})
...
function createBills(billList) {
    ...
    if (billList.length > 0) {
        ...
        $('[js-menu-bills]').removeClass('is-open');
    }
    checkBodyHidden()
...
$(document).on('submit', '[js-add-reminder-form]', function (event) {
    event.preventDefault();
    $('[js-menu-update-reminder]').css({zIndex: 10000});
...
$(document).on('click', '[data-deal-reminder]', function (e) {
    idCurrentDeal = $(e.target).closest('.custom-table__body-row').data('deal')
    $('[js-menu-update-reminder]').addClass('is-open');
})
...
function setUpdateDealInfo(data) {
    ...
    if (data.status !== 1) {
        $('.product').removeClass('menu-input_close');
        $('[u-start-date-block]').removeClass('menu-input_close');
    } else {
        $('.product').addClass('menu-input_close');
        $('[u-start-date-block]').addClass('menu-input_close');
    }
    ...
    setFunnelsToDeals(data.type, funnelsList, 'update', data.idFunnel);
...
$(document).on('change', '[check-target-status]', function (event) {
    if (+$('option:selected', this).val() === 1) {
        ...
        $(this).closest('form').find('[u-product-block]').removeClass('.menu-input_close');
...
function changeEnableBlock(t, allowed, idDeal, idClient) {
    if (Array.isArray(t)) {
        let blocks = t;
        if (blocks) {
