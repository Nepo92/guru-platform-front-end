...
$(document).on('click', '.tracker-table__settings', function (e) {
  const name = $(e.target).closest('.tracker-table__row').find('[settings-name]').html();
  const month = $('[tracker-month] option:selected').html();

  regular = $(e.target).closest('.tracker-table__row').find('.regular');
  holiday = $(e.target).closest('.tracker-table__row').find('.holiday');
  id = +$(e.target).closest('.tracker-table__row').data('id');

  $('.settings-form__personal').html(name);
  $('.settings-form__month').html(month);

  user.idByType = +$(e.target).closest('.tracker-table__row').find('.user__id').val();
  user.idCompany = +$(e.target).closest('.tracker-table__row').find('.user__idCompany').val();
  user.idDepartment = +$(e.target).closest('.tracker-table__row').find('.user__idDepartment').val();
  user.idPosition = +$(e.target).closest('.tracker-table__row').find('.user__idPosition').val();
  user.idUser = +$(e.target).closest('.tracker-table__row').find('.user__idUser').val();
  user.name = $(e.target).closest('.tracker-table__row').find('.user__name').val();
  user.positionName = $(e.target).closest('.tracker-table__row').find('.user__positionName').val();

  $('.settings__regular').val($(regular).html() != 0 ? $(regular).html() : '');
  $('.holiday__input').val($(holiday).html() != 0 ? $(holiday).html() : '');
...
$(document).on('click', '[save-settings]', saveSetting.bind(null, user, regular, holiday, counter));
...
function saveSetting(user, regular, holiday, counter = null) {
  const data = {};
  data.regularTime = +$('.settings__regular').val() * 60 * 60 * 1000;
  data.holidayTime = +$('.holiday__input').val() * 60 * 60 * 1000;
    ...
    success: function (data) {
      $.each($('.tracker-table__row'), (index, item) => {
        ...
        if (name === user.name && +$(item).data('id') === id) {
          $(item).find('.regular').html(valueReg / 60 / 60 / 1000);
          $(item).find('.holiday').html(valueHol / 60 / 60 / 1000);
        }
      });
...
function getTemplates(pesonal) {
    ...
    personalValue.forEach((item, index) => {
        const pers = item ? item[0] : null;

        if (pers) {
            departments.forEach((elem, count) => {
                if (pers.user.idDepartment === elem.id) {
                    const tr = document.createElement('tr');
                    tr.classList.add('tracker-table__row');
                    tr.setAttribute('data-id', pers.user.idUser);
                    tr.innerHTML = getTemplateRow(pers, elem);
                    const dep = document.querySelector(`.tracker__item[data-department="${elem.id}"] [tracker-list]`);
                    dep.appendChild(tr);
                }
            });
        }
            ...
            <table class="tracker__table">
                <thead>
                    <tr class="tracker-table__row--head">
                        ...
                        ${(item.title === 'Кураторство' || item.title === 'Контроль') ?
...
function getTemplateRow(item, elem) {
    ...
    return `
        ...
        <input type="hidden" name="count" class="personal__count" value="${item.count}">
        <input type="hidden" name="year" class="personal__year" value="${item.year}">
        <input type="hidden" name="month" class="personal__month" value="${item.month}">
        <input type="hidden" name="user" class="personal__user" value="${item.user}">
        <input type="hidden" name="idByType" class="user__id" value="${item.user.idByType}">
        <input type="hidden" name="idCompany" class="user__idCompany" value="${item.user.idCompany}">
        <input type="hidden" name="idDepartment" class="user__idDepartment" value="${item.user.idDepartment}">
        <input type="hidden" name="idPosition" class="user__idPosition" value="${item.user.idPosition}">
        <input type="hidden" name="idUser" class="user__idUser" value="${item.user.idUser}">
        <input type="hidden" name="name" class="user__name" value="${item.user.name}">
        <input type="hidden" name="positionName" class="user__positionName" value="${item.user.positionName}">
