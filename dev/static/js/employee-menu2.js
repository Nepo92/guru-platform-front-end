...
const needLoginArr = ['control', 'sales', 'marketing', 'supervision'];
...
$(document).on('submit', '[js-examiner-form]', function (event) {
    ...
    if (validateForm(this)) {
        ...
        createEmployee(userData, needLoginArr.includes(currentDepartment));
    }
});
...
$(document).ready(function () {
    ...
    $menuCreateUserBtn.on('click', function () {
        ...
        const menu = document.querySelector('[js-menu-create-employee]');
...
function createEmployee(userData, needLogin) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const menuExaminer = document.querySelector('[js-menu-create-examiner]');
            const wrapperExaminer = menuExaminer.querySelector('.platform-modal__wrapper');
            closeModalAnimation(menuExaminer, wrapperExaminer, false, false);
        },
        error: function (data) {
            $('[js-save-employee]').prop("disabled", false);
            $('[js-save-examiner]').prop("disabled", false);
...
function addRowToTable(data, needLogin) {
    var seloption = "";
    var arr = $typeList['' + currentDepartment + ''];
    $.each(arr, function (index, value) {
    ...
    const employeeTemplate = {
        data, seloption, needLogin,
    };

    const employeeRow = templateEmloyeeRow(employeeTemplate);

    table.get(0).children[0].insertAdjacentHTML('afterend', employeeRow);

    initDatePicker();
}

function templateEmloyeeRow(employeeTemplate) { 
    const { data, seloption, needLogin } = employeeTemplate;

    console.log(needLogin);

    return '<tr data-employee="' + data.idEmployee + '" class="custom-table__body-row staff-table__body-row">' +
        '                        <input type="hidden" u-id value="' + data.idEmployee + '" name="id" required>' +
        '                        <td class="custom-table__body-col staff-table__id">' + data.idEmployee + '</td>' +
        ...
        '                                   placeholder="—">' +
        '                        </td>' +
        (needLogin ? '                        <td class="custom-table__body-col custom-table__body-col_xlarge staff-table__login">' +
