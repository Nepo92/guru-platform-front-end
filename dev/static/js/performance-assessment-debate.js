...
function setRateItemInfo(data) {
    ...
    $.each(data.comments, function(index, comment) {
        ...
        if ((documents.length != 0 || comment.text != null) || comment.type == 'manager') {
            comments +=
                '<div class="comment-debate ' + (comment.type == 'examiner' ? 'comment__wrapper_400' : 'comment__wrapper_400 comment__wrapper_right') + '">' +
                '   <div class="comment__wrapper ' + (comment.type == 'examiner' ? ' comment__wrapper_black' : '') + '">' +
                '<div class="comment-debate__date ' + (comment.type == 'examiner' ? 'comment-debate__date--white' : '') + '">' + comment.createDate + '</div>' +
                ...
                ((comment.text) ? '<div class="comment__text">' + comment.text + '</div>' : '') +
