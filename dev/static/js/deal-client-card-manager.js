...
function setUpdateStreams(idCourse) {
  ...
  $.ajax({
    type: "POST",
    url: "../performance-assessment/getStreamsByIdCourse",
    data: formData,
    processData: false,
    contentType: false,
    cache: false,
...
$(document).on('submit', '[js-deal-form]', function (event) {
  ...
  if (validateForm(this)) {
    ...
    dealData.idManager = +$(this).closest('[js-menu-create-deal]').find('[js-deal-form-manager]').attr('value');
