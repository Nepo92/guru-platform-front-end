            ...
            if (typeof type !== typeof undefined && type !== false) {
                ...
                if (type === 'video') {
                    ...
                    replaced =
                        ...
                        '<label class="banners__btn" for="files-mobile' + LESSON_BLOCK_COUNTER + '">Загрузить мобайл (375 x 220px)</label>' +
                        '</div>' +
                        ...
                        '<label class="banners__btn" for="files-desktop' + LESSON_BLOCK_COUNTER + '">Загрузить десктоп (1180 x 220px)</label>' +
                        '</div>' +
                        '</div>' +
        ...
        blocks.then((result) => {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        const wrapper = menu.qerySelector('.platform-modal__wrapper');
                        closeModalAnimation(menu, wrapper, false, false);
...
$(document).on('click', '.preview__delete', function (event) {
    ...
    $.each(inputs, (index, item) => {
        ...
        const isInputVerion = $(item).is(`.${version}`);
        const isPreviewVersion = $(preview).is(`.preview-${version}`);

        if (isInputVerion && isPreviewVersion) {
            item.files.length = 0;
            $(item).val('');
        }
    });
...
function renderTariffs(tariffs) {
    $('[tariffs-list]').children().remove();

    const tariffItem = $.map(tariffs.reverse(), (tariff, index) => {
        return `
    ...
    $('[tariffs-list]').append(tariffItem);

    addNewTariffButton();
}

function addNewTariffButton() {
    const createTariff = () => {
        return `
				<div new-tariff class="tariff-block create-tariff">
    ...
    if (!$('.create-tariff').get(0)) {
        $('[tariffs-list]').append(createTariff());
    }
}

$(document).on('click', '[update-tariff]', function (e) {
    tariff.updateTariff(e);
})

$(document).on('click', '[new-tariff]', function (e) {
    tariff.addNewTariff(e);
});