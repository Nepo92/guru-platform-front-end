...
const dealAPI = {
    getCoursesList: function (courses) {
        $coursesList = courses;
    },
...
$(document).on('change', '[js-deal-form-product]', function (event) {
    ...
    // getData.startDate($('option:selected', this).data('id'));
...
function setDealInfo(data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const sortData = sortingArray(data.reminders);
            renderTasks(sortData);
        },
        error: function (data) {
            $('[js-update-deal-btn]').prop("disabled", false);
            $('[js-update-deal]').prop("disabled", true);
...
$(document).on('submit', '[js-deal-form]', function (event) {
    ...
    if (validateForm(this)) {
        ...
        const idTariff = +$(this).closest('[js-menu-create-deal]').find('[tariff-selected]').data('id');
...
function createDeal(dealData) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('[js-deal-form]').find('.tariff-content').remove();
...
function setFunnelsToDeals(dealType, funnels, settings = null, idFunnel = null) {
    ...
    $.each(funnelsObj, (index, item) => {
        ...
        if (type === dealType || type === deal) {
            ...
            const dealWrapper = $('.deal-type');

            if (!$('.create-deal__funnel').get(0)) {
                $(chooseFunnel()).insertAfter(dealWrapper);
            }
...
$(document).on('click', '.select__option', function (e) {
    ...
    if (!selectClose) {
        ...
        const current = $(t).closest('[select-tariff]').find('.select__head');

        current.html(selectedText);
        current.attr('data-id', $(t).data('id'))
...
$(document).on('click', '[select-tariff]', function (e) {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (!selectClose) {
        ...
        const body = $(t).closest('[select-tariff]').find('.select__body');
...
$(document).on('click', function (e) {
    if (selectClose === false) {
        if (!$(e.target).is('.select__option--tariff')) {
            $('.select__body').css({display: 'none'});
            selectClose = true;
        }
    }
