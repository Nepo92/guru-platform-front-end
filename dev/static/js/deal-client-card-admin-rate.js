...
$(document).on('click', '[js-update-deal-btn]', function (event) {
    ...
    $('[js-menu-update-deal]').css({zIndex: '10000'})
...
function setUpdateStreams(idCourse) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('[js-deal-form-start-date]').append(seloption);
        },
        error: function (data) {
        }
    });
...
$(document).on('change', '[check-target-status]', function (event) {
    if ($('option:selected', this).val() == 1) {
        ...
        $('[not-target]').find('[product-block]').removeClass('menu-input_close');
        $('[not-target]').find('[start-date-block]').removeClass('menu-input_close');
...
$(document).on('click', '[js-menu-create-deal-close-btn]', function () {
...
$(document).on('change', '[js-deal-form-sale-type]', function () {

    if ($(this).closest('[js-menu-create-deal]').get(0)) {
        setFunnelsToDeals($(this).val(), funnels, 'create-deal');
    }
